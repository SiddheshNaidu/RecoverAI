/**
 * RegisterPatientPage — Staff-side patient onboarding → Supabase
...
export default function RegisterPatientPage() {
 ...
 <p className="font-inter text-ink-muted">Creates a live record in Supabase</p>
 </div>
 </header>

 {submitError && (
 <div className="rounded-2xl p-4 bg-error/10 text-error text-sm font-inter" role="alert">
 {submitError}
 </div>
 )}
 ...
 type="button"
 onClick={() => removeMed(i)}
 className="w-10 h-10 rounded-full bg-error/10 text-error flex items-center justify-center hover:bg-error/20 transition-colors shrink-0"
 >
 ...
 boxShadow:
 "6px 6px 16px rgba(27, 67, 50,0.4), -3px -3px 10px rgba(82, 183, 136,0.3), inset 0 1px 0 rgba(255,255,255,0.15)",
 }}
 >
 ...
 <p className={`font-inter text-base ${i === pipeStep ? "text-primary font-medium" : "text-ink-muted"}`}>
 {msg}
 </p>
 </div>
 ))}
 ...
 {createdId && (
 <Link
 to={`/receptionist/patient/${createdId}`}
 className="flex-1 min-w-[140px] text-center py-4 rounded-2xl bg-primary/15 text-primary font-heading font-bold hover:bg-primary/25 transition-colors"
 >
 View patient
 </Link>
 )}
 <Link
 to="/receptionist"
 className="flex-1 min-w-[140px] text-center py-4 rounded-2xl bg-surface-high font-heading font-bold text-ink hover:bg-surface-dim transition-colors"
 >
 Back to Dashboard
 </Link>
 <button
 ...
 className="flex-1 min-w-[140px] py-4 rounded-2xl font-heading font-bold text-white"
 style={{
 background: "linear-gradient(135deg, #3d5442, #6b8f71)",
 boxShadow: "5px 5px 12px rgba(27, 67, 50,0.35), -2px -2px 8px rgba(82, 183, 136,0.25)",
 }}
 >
 Register Another
 </button>
 ...
 <style>{`
 .field-input { width: 100%; padding: 1rem 1.25rem; background: #f7f6ef; border-radius: 0.75rem; border: 1px solid rgba(194,200,192,0.3); outline: none; font-family: 'Inter', sans-serif; font-size: 1rem; color: #1a1f1b; transition: border-color 0.2s; }
 .field-input:focus { border-color: #3d5442; }
 `}</style>
 </main>
 );
}
