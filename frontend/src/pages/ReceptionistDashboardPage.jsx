...
import { compareRiskLevel, riskLevelToOrb } from "../lib/recoveryMappers";
...
export default function ReceptionistDashboardPage() {
  ...
  const riskFirst = useMemo(() => [...filtered].sort(compareRiskLevel), [filtered]);

  const critical = patients.filter((p) => String(p.risk_level).toUpperCase() === "CRITICAL").length;

  if (loading) {
    return <div className="p-8 text-center font-inter text-ink-muted">Loading dashboard…</div>;
  }
  if (error) {
    ...
    <main className="min-h-screen bg-surface pb-12">
      ...
      >
        <div className="max-w-[1100px] mx-auto flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div>
            <p className="font-inter text-white/75 text-sm uppercase tracking-widest font-semibold mb-2">
            ...
            {critical > 0 && (
              <div className="flex items-center gap-2 mt-3">
                ...
                <p className="font-inter text-sm text-white/85">
      ...
      <div className="max-w-[1180px] mx-auto px-6 md:px-12 lg:px-20 pt-8 flex flex-col gap-10">
        ...
        <section className="animate-fade-up-delay">
          ...
          {appointments.length === 0 ? (
            <p className="font-inter text-ink-muted text-sm">No appointments scheduled for today.</p>
          ) : (
        ...
        <section className="flex flex-col gap-5 animate-fade-up-delay">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="relative w-full sm:w-64">
              <span
                className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-ink-muted"
                style={{ fontSize: "18px" }}
              >
                search
              </span>
    ...
    >
      ...
      >
        ...
        <span
          ...
          style={{ fontSize: "10px", letterSpacing: "0.02em", color: isCritical ? "#ba1a1a" : "#424842" }}
        >
          Day
        </span>
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-3 mb-1">
          <h3 className="font-heading font-bold text-ink text-[1.05rem] leading-tight">{p.name}</h3>
          <StatusOrb risk={orbRisk} size="sm" />
        </div>
        <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5">
          <span className="font-inter text-[0.92rem] text-ink-muted leading-snug">{p.procedure || "—"}</span>
          <span
            className="font-inter text-[0.9rem] font-semibold"
            style={{
